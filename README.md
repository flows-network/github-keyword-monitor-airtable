...
[Deploy this function on flows.network](#deploy-your-own-code-review-bot-in-3-simple-steps), and you can save the GitHub issues that mentions a keyword setting by you to an airtable table. For example, if you set the keyword as WasmEdge, any GitHub issues includes WasmEdge will be saved to the Airtable table.

## How it works

This function is triggered by time. At the specified time, the bot searches for all GitHub issues from the past 24 hours, filters them based on your chosen keyword, and save the resulits into an Airtable table.
...
A pre-build Airtable table to save the resuls. We recommend you to use the template we already build for you.
